...
async function generateMeaningAndExampleWithGemini(
  ...
  const prompt = [
    ...
    '- ANTI-CLUSTERING rule: do NOT default to 4. Across many items, the realistic distribution is roughly 5%/15%/40%/30%/10% for levels 1/2/3/4/5. If you find yourself rating mostly 4s, you are wrong — re-evaluate.',
    '- DEFAULT TO 3 when the item is moderately common but you are unsure. 3 is the safest middle. Only go to 4 if you are genuinely confident it is in the top tier of daily-use items in its category.',
    '- FINAL SANITY CHECK before returning: if you chose 4 or 5, ask yourself "Is this truly more common than at least 60% of items in this category?" If not, drop to 3. If you chose 1, ask "Is this really archaic/extinct?" If not, raise to 2.',
    '- Examples of items that look common but should NOT be 4: "extensively" (→3), "deploy" (→3, technical), "resupply" (→2-3, military/business jargon), "conducted" (→3, formal register), "vessels" (→3, formal/nautical). Only true daily basics deserve 4–5.',
