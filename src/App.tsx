...
type GeminiAutofillResult = {
  ...
  itemType?: ItemType
  usageFrequency?: number      // 1(매우드묾)~5(매우빈번) 사용빈도
}
...
async function generateMeaningAndExampleWithGemini(
  ...
  const prompt = [
    ...
    '      "description": string,    // 1-2 sentence Korean explanation of when/how to use this meaning',
    ...
    '  ▸ If itemType is "expression", calibrate against other expressions/phrasal-verbs/collocations. NOTE: basic phrasal verbs ("look up", "look at", "look into", "get up", "give up", "take off", "let go", "find out") are EXTREMELY COMMON — rate them 4–5, NOT 2. Phrasal verbs are core daily English, not formal/dated.',
    '    • 5 = extremely common, used daily (e.g., "thank you", "I don\'t know", "by the way", "look at", "look up", "get up", "take a look", "find out", "go on")',
    '    • 4 = frequently heard (e.g., "look into", "look over", "let go", "as a matter of fact", "in the long run", "to be honest", "look down on")',
    '    • 3 = moderately used (e.g., "bear in mind", "on behalf of", "look through", "get along with")',
    '    • 2 = uncommon, formal, or dated (e.g., "come forth", "the proper gratitude", "I suppose you aren\'t aware", "henceforth")',
    ...
    '- DISTRIBUTE realistically across 1–5. Do NOT cluster everything at one level. A healthy spread roughly looks like 5%/15%/30%/35%/15% for levels 1/2/3/4/5 — meaning 4 is normal for common items, NOT rare.',
    '- DO NOT over-correct downward. If a word/phrase is genuinely common in daily use, rate it 4 or 5 even if you feel uncertain. Common phrasal verbs and everyday vocabulary deserve 4–5.',
    '- Calibration check by example (use these as concrete reference points):',
    '    • Vocabulary: "extensively"=3, "deploy"=3, "resupply"=2, "conducted"=3, "vessels"=3, "aware"=4, "gratitude"=4, "people"=5, "go"=5',
    '    • Expression: "look at"=5, "look up"=5, "look into"=4, "look through"=3, "let go"=4, "by the way"=5, "as a matter of fact"=4, "come forth"=2',
    '    • Idiom: "break a leg"=5, "piece of cake"=5, "spill the beans"=4, "cut corners"=3, "mad as a hatter"=2',
    '- Reserve 1 for genuinely archaic/extinct usage and 5 for absolute top-tier within the category. Be willing to use ALL five levels.',
  ...
  try {
    ...
    const parsedItemType =
      parsed.itemType === 'vocabulary' || parsed.itemType === 'expression' || parsed.itemType === 'idiom'
        ? parsed.itemType
        : undefined
    const rawFreq = Number((parsed as Record<string, unknown>).usageFrequency)
    const usageFrequency = [1, 2, 3, 4, 5].includes(rawFreq) ? rawFreq : undefined
    if (!meaningKo || !exampleEn) return { result: null, error: 'empty-fields' }
    return { result: { meaningKo, altMeaningsKo, exampleEn, exampleKo, examples, definitionHint, itemType: parsedItemType, usageFrequency } }
  } catch (err) {
...
function App() {
  ...
  const autoFillFromEnglish = async (forceUpdate = false, provider: AiProvider = formAiProvider) => {
    ...
    let resolvedFrequency: (1 | 2 | 3 | 4 | 5) | undefined
    ...
    if (geminiResult) {
      ...
      if (geminiResult.usageFrequency) resolvedFrequency = geminiResult.usageFrequency as 1 | 2 | 3 | 4 | 5
      usedGemini = true
